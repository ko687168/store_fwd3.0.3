...
static void calculate_chdir_dir(struct vsf_session* p_sess,
    int anon, struct mystr* p_userdir_str,
    ...
    int do_chroot, int anon)
{
    ...
    if (newpid == 0)
    {
        ...
        str_alloc_text(&debug_str, "common_do_login:");
        str_append_str(&debug_str, &chroot_str);
        str_append_text(&debug_str, ", ");
        str_append_str(&debug_str, &chdir_str);
...
calculate_chdir_dir(
    ...
    int anon_login, struct mystr* p_userdir_str,
    struct mystr* p_chroot_str,
    struct mystr* p_chdir_str,
    const struct mystr* p_user_str,
    const struct mystr* p_orig_user_str)
{
    if (!anon_login)
    {
        ...
        str_alloc_text(&debug_str, "calculate_chdir_dir:");
        str_append_str(&debug_str, p_orig_user_str);
        str_append_text(&debug_str, ", ");
        str_append_str(&debug_str, p_user_str);
        str_append_text(&debug_str, ", ");
        ...
        if (p_user == 0)
        {
            if (!anon_login && tunable_local_root)
            {
                ...
                str_alloc_text(&debug_str, "calculate_chdir_dir: chroot:");
                str_append_str(&debug_str, p_chroot_str);

                vsf_log_line(p_sess, kVSFLogEntryDebug, &debug_str);

                str_copy(p_chdir_str, p_chroot_str);
                return;
            }

            die2("cannot locate user entry2:", str_getbuf(p_user_str));
        }

        // vsf_log_line(p_sess, kVSFLogEntryConnection, (struct mystr *)p_user_str);
        // vsf_log_line(p_sess, kVSFLogEntryConnection, (struct mystr *)p_user_str);
        // vsf_log_line(p_sess, kVSFLogEntryConnection, (struct mystr *)p_user_str);
        // vsf_log_line(p_sess, kVSFLogEntryConnection, (struct mystr *)p_user_str);
